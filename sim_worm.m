function [forcess,positions,x] = sim_worm(N,N_chains,T,eLJ_inter,eLJ_intra,spring_coeff,bend_coeff,active_coeff,sigma,steps)
...
% sigma: equilibrium distance (default = 2^(1/4)f from modified L-J force)
...
% initial relaxation steps at extra small dt
n_pre_steps = 10000;
step_grad = linspace(dt/10000000,dt,n_pre_steps);
for pre_step = 1:n_pre_steps
    [x,~] = steepest_descent (N,N_chains,x,step_grad(pre_step),eLJ_inter,eLJ_intra,sigma,spring_coeff,bend_coeff,active_coeff,T); % calculate new positions based on forces
end

% main dynamics loop
for step_i=1:steps

    [x,~,F] = steepest_descent (N,N_chains,x,dt,eLJ_inter,eLJ_intra,sigma,spring_coeff,bend_coeff,active_coeff,T); % calculate new positions based on forces
